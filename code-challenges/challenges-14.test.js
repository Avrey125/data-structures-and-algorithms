...
const toTitleCase = (arr) => {
  // Solution code here...
  let stuff = arr.map(value => {
    let thangs = value.charAt(0).toUpperCase();
...
let biggerThanLuke = (arr) => {
  let starWar = arr.filter(value => parseInt(value.mass) > parseInt(arr[0].mass));

  return starWar.map(value => value.name).join(' - ');
};
