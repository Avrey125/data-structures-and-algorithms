...
const onlyOddChars = (str) => {
  // Solution code here...
  const arr = str.split('');
  const stuff = arr.filter((el, index) => {
    if (index % 2 === 1){
      return el;
    }
  });
  const strOdd = stuff.join('');
